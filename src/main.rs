...
fn run_direct(sim: &mut NBodySimulation2D) {
    for i in 0..100 {
        let img_filename: &str = &*format!("data/frames/img{:04}.png", i);
        let res: Result<(), Box<dyn std::error::Error>> = create_plot(
            img_filename, WIDTH, HEIGHT, &sim);
        match res {
            Ok(v) => v,
            Err(e) => println!("Error: {:?}", e),
        }   
        nbody_direct_2d(sim, 1.0);
    }

    compile_mp4();
}

fn run_direct_2d_test() {
...
fn run_direct_2d_galaxy() {
    let mut sim: NBodySimulation2D = generate_galaxy(100);
    run_direct(&mut sim);
}

fn run_barnes_hut() {
    let x: Vec<f32> = vec![100., 250., 400.];
    let y: Vec<f32> = vec![100., 250., 400.];
